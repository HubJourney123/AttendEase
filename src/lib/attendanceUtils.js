...
export const generatePDF = (
  ...
  try {
    ...
    // Prioritize pdfInfo over classData
    const department = pdfInfo?.department || classData?.department || 'Department Name';
    const year = pdfInfo?.year || classData?.year || 'Unknown';
    const term = pdfInfo?.term || classData?.term || 'Unknown';
    pdf.text(department, pageWidth / 2, 17, { align: 'center' });
    pdf.text(`${year} ${term} B. Sc. Engineering`, pageWidth / 2, 22, { align: 'center' });
    ...
    pageGroups.forEach((group, pageIndex) => {
      ...
      if (pageIndex === pageGroups.length - 1) {
        ...
        pdf.setFontSize(9);
        pdf.text(`Course Code: ${classData?.courseCode || 'N/A'}`, 15, footerY);
        pdf.text(`Course Title: ${classData?.courseName || 'N/A'}`, 15, footerY + 4);
        pdf.text(`Batch: ${classData?.batch || 'N/A'}`, 15, footerY + 8);

        const sigX = pageWidth - 65;
        pdf.line(sigX, footerY, sigX + 50, footerY);
        pdf.text("Course Teacher", sigX + 25, footerY + 4, { align: "center" });
      }
    });

    // ---------- SAVE ----------
    const fileName = `Roll_Sheet_${classData?.courseCode || 'Unknown'}_${classData?.batch || 'Unknown'}.pdf`;
    pdf.save(fileName);
